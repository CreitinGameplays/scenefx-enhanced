...
static bool link_shaders(struct fx_renderer *renderer) {
	...
	// Liquid glass shader
	if (!link_liquid_glass_program(&renderer->shaders.liquid_glass)) {
		wlr_log(WLR_ERROR, "Could not link liquid_glass shader");
		goto error;
	}
...
error:
	...
	glDeleteProgram(renderer->shaders.liquid_glass.program);
